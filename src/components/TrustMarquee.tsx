 ...
 */
const ITEMS = [
  "MIT", "Harvard", "Oxford", "Stanford", "Cambridge",
  "MBTI Assessment", "IQ Profiling", "Career Intelligence",
  "AI-Powered", "SAT Prep", "IELTS Ready", "NUS Singapore",
  "ETH Zürich", "Toronto", "UCL", "KAIST",
];
...
export function TrustMarquee() {
  const row = ITEMS.map((label, i) => (
    <span key={i} className="inline-flex shrink-0 items-center">
      <span className="text-xs font-medium text-muted-foreground/70 whitespace-nowrap">{label}</span>
      {DOT}
    </span>
  ));
