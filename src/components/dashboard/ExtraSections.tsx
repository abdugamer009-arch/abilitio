  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("[ExtraSections]", err);
      }
    })();
    return () => {
      cancel = true;
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {(skills ?? Array.from({ length: 7 })).map((s: SkillSnapshot | undefined, i) =>
            s ? (
              <div
                key={s.key}
                className="group relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-secondary/30 to-background/40 p-5 transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-[0_8px_24px_-10px_oklch(0.55_0.22_295_/_0.3)]"
              >
                <GlowBlob
                  className="-right-6 -top-6 h-16 w-16 opacity-0 blur-2xl transition-opacity duration-300 group-hover:opacity-40"
                  alpha={0.8}
                />
                ...
                <div className="mt-2 h-2 overflow-hidden rounded-full bg-secondary/60">
                  <div
                    ...
                    style={{
                      width: `${s.progress}%`,
                      boxShadow: "0 0 10px oklch(0.65 0.22 295 / 0.5)",
                    }}
                  />
                </div>
              </div>
              ...
              <div
                key={i}
                className="h-32 rounded-2xl border border-border/40 bg-secondary/20 animate-pulse"
              />
            ),
          )}
        </div>
                ...
                <BarChart data={report.improvements}>
                  ...
                  <Tooltip
                    contentStyle={{
                      ...
                      border: "1px solid oklch(0.30 0.04 295)",
                      borderRadius: 12,
                    }}
                  />
          ...
          <div className="rounded-2xl border border-border/60 bg-secondary/30 p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
              {report.suggestions.map((s, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 rounded-xl border border-border/60 bg-background/30 px-3 py-2"
                >
  ...
  return (
    <div className="space-y-6">
      <div className="glass relative overflow-hidden rounded-3xl p-8 text-center">
        <GlowBlob
          className="left-1/2 top-0 h-40 w-72 -translate-x-1/2 opacity-20 blur-3xl"
          alpha={0.8}
        />
