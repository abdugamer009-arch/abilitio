...
export const getPrincipalDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<PrincipalDashboardDTO> => {
    ...
    if (dominant && dominant.count > 0) {
      insights.push(
        `Your school has a strong concentration of ${dominant.label.toLowerCase()}-oriented students (${dominant.count}).`,
      );
    }
    const strongClass = classBreakdown
      .map((c) => ({
        name: c.className,
        top: c.buckets.slice().sort((a, b) => b.count - a.count)[0],
      }))
      .filter((c) => c.top && c.top.count >= 3)[0];
    if (strongClass) {
      insights.push(
        `Class ${strongClass.name} shows exceptionally strong ${strongClass.top!.label.toLowerCase()} potential.`,
      );
    }
    if (completionRate < 60 && students.length > 0) {
      insights.push(
        `Only ${completionRate}% of students have completed their assessments — encourage participation for sharper insights.`,
      );
    }
    if (!insights.length)
...
function reasonFor(bucket: string): string {
  switch (bucket) {
    case "engineering":
      return "Strong mathematical ability, analytical thinking, engineering compatibility.";
    case "business":
      return "Leadership potential, communication skills, entrepreneurship indicators.";
    case "medical":
      return "Scientific reasoning, empathy, healthcare compatibility.";
    case "creative":
      return "Creativity, imagination, design potential.";
    case "communication":
      return "Verbal fluency, persuasion, audience awareness.";
    case "research":
      return "Deep curiosity, analytical rigor, comfort with ambiguity.";
    case "technology":
      return "Logical reasoning, technical aptitude, pattern recognition.";
    case "leadership":
      return "Decision-making, vision, ability to mobilize teams.";
    default:
      return "Strong alignment across multiple dimensions.";
  }
}
...
export const listSpecializedClasses = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<{ classes: SpecializedClassDTO[] }> => {
    ...
    const { data: rows } = await supabaseAdmin
      .from("school_specialized_classes")
      .select("*")
      .eq("school_id", school.id)
      .order("focus");
    type SpecClassRow = {
