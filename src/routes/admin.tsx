...
const PURPLE = ["oklch(0.65 0.24 295)", "oklch(0.75 0.18 320)", "oklch(0.55 0.22 280)", "oklch(0.70 0.20 310)", "oklch(0.62 0.20 270)", "oklch(0.78 0.14 300)", "oklch(0.50 0.20 290)", "oklch(0.68 0.22 330)"];
...
function AdminDashboardPage() {
  ...
  useEffect(() => {
    if (loading) return;
    if (!user) { navigate({ to: "/auth", search: { mode: "login", next: "/admin" } }); return; }
    (async () => {
      try {
        const { isAdmin } = await checkFn();
        if (!isAdmin) { setForbidden(true); setReady(true); return; }
        const [a, u] = await Promise.all([analyticsFn(), usersFn()]);
        setAnalytics(a);
        setUsers(u);
        setReady(true);
      } catch {
        setForbidden(true); setReady(true);
      }
    })();
  ...
  if (loading || !ready) {
    return <PageShell><div className="px-6 pt-32 text-center text-sm text-muted-foreground">Loading admin dashboard…</div></PageShell>;
  }
      ...
      <PageShell>
        <section className="px-6 pt-24 pb-24">
          <div className="mx-auto max-w-md glass rounded-3xl p-10 text-center">
            ...
            <Link to="/" className="mt-6 inline-flex rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:glow-purple">Go home</Link>
          </div>
        </section>
      </PageShell>
    );
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Founder-level analytics & user management</p>
            </div>
          </div>
            ...
            <>
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                <GlassChart title="Signups — last 7 days">
                  <ResponsiveContainer width="100%" height={260}>
                    <LineChart data={analytics.weeklySignups}>
                      ...
                      <Line type="monotone" dataKey="count" stroke="oklch(0.65 0.24 295)" strokeWidth={3} dot={{ fill: "oklch(0.65 0.24 295)" }} />
                    </LineChart>
                  </ResponsiveContainer>
                </GlassChart>
                ...
                <GlassChart title="Most popular careers">
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={analytics.popularCareers} layout="vertical" margin={{ left: 80 }}>
                      <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" opacity={0.3} />
                      <XAxis type="number" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} allowDecimals={false} />
                      <YAxis type="category" dataKey="name" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} width={120} />
                      <Tooltip contentStyle={{ background: "var(--background)", border: "1px solid var(--border)", borderRadius: 12 }} />
                ...
                <GlassChart title="Personality types distribution">
                  <ResponsiveContainer width="100%" height={260}>
                    <PieChart>
                      <Pie data={analytics.popularMbti} dataKey="count" nameKey="type" innerRadius={50} outerRadius={90} paddingAngle={2}>
                        {analytics.popularMbti.map((_, i) => <Cell key={i} fill={PURPLE[i % PURPLE.length]} />)}
                      </Pie>
                      <Tooltip contentStyle={{ background: "var(--background)", border: "1px solid var(--border)", borderRadius: 12 }} />
              ...
              <div className="mt-8">
                ...
                <div className="mt-4 overflow-x-auto rounded-3xl border border-border/60 bg-secondary/20 backdrop-blur-xl">
                  <table className="w-full min-w-[800px] text-sm">
                    ...
                    <tbody>
                      {filtered.map((u) => (
                        <tr key={u.id} className={`border-b border-border/30 ${u.is_banned ? "opacity-60" : ""}`}>
                          <td className="px-4 py-3 font-medium">
                            {(u.name || u.surname) ? `${u.name} ${u.surname}`.trim() : "—"}
                            {u.is_banned && <span className="ml-2 rounded-full bg-destructive/15 px-2 py-0.5 text-[10px] text-destructive">banned</span>}
                          </td>
...
function StatCard({ icon: Icon, label, value, valueText, hint }: { icon: any; label: string; value?: number; valueText?: string; hint?: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-secondary/40 to-background/40 p-5 backdrop-blur-xl"
      ...
      <div className="pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full opacity-40 blur-2xl"
...
function GlassChart({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-3xl border border-border/60 bg-secondary/20 p-6 backdrop-blur-xl"
  ...
  return (
    <button
      ...
      className={`inline-flex h-8 w-8 items-center justify-center rounded-full border transition-all hover:-translate-y-0.5 disabled:opacity-40 ${
        danger ? "border-destructive/40 bg-destructive/10 text-destructive hover:bg-destructive/20" : "border-border/60 bg-secondary/40 hover:bg-secondary"
      }`}
    >
      {children}
    </button>
