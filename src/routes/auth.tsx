          ...
          <div className="glass mt-8 rounded-3xl p-7">
            <div className="mb-6 grid grid-cols-2 gap-1 rounded-full bg-secondary/60 p-1">
              {(["login", "signup"] as const).map((m) => (
                <button key={m} onClick={() => setTab(m)} className={`rounded-full py-2 text-sm transition-all ${tab === m ? "bg-primary text-primary-foreground glow-purple" : "text-muted-foreground"}`}>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <Input type="password" placeholder={t.auth.password} value={password} onChange={setPassword} />
