...
import {
  ...
  togglePinMessage,
  getCommunityDailyQuestion,
  setCommunityDailyQuestion,
  fetchMessageAuthors,
  type CommunityDTO,
  ...
  head: () => ({
    meta: [
      { title: "Community — Abilitio" },
      {
        name: "description",
        content:
          "Connect with students and mentors, share progress, and get inspired on your talent-discovery journey.",
      },
    ],
  }),
...
function CommunityPage() {
  ...
  useEffect(() => {
    if (loading) return;
    if (!user) {
      navigate({ to: "/auth", search: { mode: "login", next: "/community" } });
      return;
    }
    getMine().then((d) => {
      setData(d);
          ...
          return (
            <li key={c.id}>
              ...
              >
                ...
                <span className="shrink-0 rounded-full bg-secondary/70 px-1.5 py-0.5 text-[10px] tabular-nums text-muted-foreground">
                  {c.member_count}
                </span>
              </button>
            </li>
...
function CommunityChat({
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`community:${community.id}`, { config: { presence: { key: currentUserId } } })
      .on(
        "postgres_changes",
        {
          event: "INSERT",
          ...
          <div className="flex-1">
            ...
            {editingDaily ? (
              <div className="mt-2 flex gap-2">
                ...
                <button
                  onClick={async () => {
                    await setDailyFn({ data: { communityId: community.id, question: dailyDraft } });
                    setDaily({ question: dailyDraft });
          ...
          <div className="space-y-1.5">
            {pinned.map((m) => (
              <div key={m.id} className="text-xs text-foreground/90">
                <span className="font-medium">{authors[m.user_id]?.name ?? "—"}: </span>
                {m.content}
              </div>
            ))}
          </div>
            ...
            <div className="space-y-4">
              {messages.map((m) => (
                <MessageRow
                  ...
                  onTogglePin={async () => {
                    await pinFn({ data: { id: m.id, pin: !m.is_pinned } });
                    setMessages((p) =>
                      p.map((x) => (x.id === m.id ? { ...x, is_pinned: !m.is_pinned } : x)),
                    );
                  }}
                />
              ))}
