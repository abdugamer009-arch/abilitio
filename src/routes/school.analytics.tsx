    ...
    return (
      <PageShell>
        <div className="mx-auto max-w-md px-6 pt-32 text-center">
          ...
          <Link to="/school/register" className="mt-6 inline-flex rounded-full bg-primary px-6 py-3 text-sm text-primary-foreground hover:glow-purple">{t.school.registerSchool}</Link>
        </div>
      </PageShell>
    );
  }
  if (!d) return <PageShell><div className="px-6 pt-32 text-center text-sm text-muted-foreground">{t.school.loadingAnalytics}</div></PageShell>;
        ...
        <div className="mx-auto max-w-6xl">
          <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-bold gradient-text">{t.school.talentIntelligence}</h1>
            </div>
            <div className="flex gap-2">
              <Link to="/school/class-builder" className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground hover:glow-purple"><Wand2 className="h-4 w-4" />{t.school.aiClassBuilder}</Link>
              <Link to="/school/dashboard" className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm hover:bg-secondary/50">{t.school.schoolOverview}</Link>
            </div>
          </header>
          ...
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <Stat icon={Brain} label={t.school.personalityTypes} value={d.personalityDistribution.length} />
            <Stat icon={GraduationCap} label={t.school.universityTracks} value={d.universityDistribution.length} />
          </div>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            ...
            <Glass title={t.school.personalityDistribution} subtitle={t.school.personalityDistributionSub}>
              <div className="h-72">
                <ResponsiveContainer>
                  <PieChart>
            ...
            <Glass title={t.school.universityMajorInterest} subtitle={t.school.mostRecommendedMajors}>
              <ul className="space-y-2 text-sm">
                ...
                {!d.universityDistribution.length && <li className="text-xs text-muted-foreground">{t.school.noData}</li>}
              </ul>
            </Glass>
          </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {d.classHeatmap.map((row) => (
                <div key={row.className} className="rounded-2xl border border-border bg-secondary/30 p-4">
                  <div className="text-sm font-semibold">{t.school.classLabel} {row.className}</div>
              ...
              {!d.classHeatmap.length && <div className="rounded-2xl border border-dashed border-border p-6 text-center text-xs text-muted-foreground md:col-span-2">{t.school.noClassData}</div>}
            </div>
          </div>

          {/* At-risk */}
          <div className="glass mt-6 rounded-3xl p-6">
            <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-primary" /><h3 className="text-sm font-semibold">{t.school.atRiskTitle}</h3></div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {atRisk.length ? atRisk.map((s) => (
                <div key={s.user_id} className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">{t.school.currently} <span className="text-foreground/80">{s.class_name}</span></div>
                  <div className="text-xs">{t.school.recommendedLabel} <span className="text-primary font-medium">{s.top_career?.name}</span> · {s.top_career?.score}% {t.school.confidence}</div>
                </div>
              )) : <div className="rounded-2xl border border-dashed border-border p-6 text-center text-xs text-muted-foreground md:col-span-2">{t.school.noMisalignments}</div>}
            </div>
          </div>

          {/* AI Insights */}
          <div className="glass mt-6 rounded-3xl p-6">
            <div className="flex items-center gap-2"><Lightbulb className="h-4 w-4 text-primary" /><h3 className="text-sm font-semibold">{t.school.aiInsights}</h3></div>
            <ul className="mt-3 space-y-2 text-sm">
              {d.careerDistribution[0] && (
                <li className="rounded-xl border border-primary/20 bg-primary/5 px-4 py-3">
                  {fill(t.school.insightCareer, { career: d.careerDistribution[0].name, count: d.careerDistribution[0].count })}
                </li>
              )}
              {d.personalityDistribution[0] && (
                <li className="rounded-xl border border-primary/20 bg-primary/5 px-4 py-3">
                  {fill(t.school.insightPersonality, { type: d.personalityDistribution[0].type, count: d.personalityDistribution[0].count })}
                </li>
              )}
              {d.cognitiveDistribution[0] && (
                <li className="rounded-xl border border-primary/20 bg-primary/5 px-4 py-3">
                  {fill(t.school.insightCognitive, { tier: [...d.cognitiveDistribution].sort((a, b) => b.count - a.count)[0].tier })}
                </li>
              )}
            </ul>
          </div>
