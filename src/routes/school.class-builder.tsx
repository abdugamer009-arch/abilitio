...
export const Route = createFileRoute("/school/class-builder")({
  head: () => ({
    meta: [
      { title: "AI Class Builder — Abilitio" },
...
function ClassBuilderPage() {
  ...
  const [result, setResult] = useState<{
    focus: string;
    students: { user_id: string; name: string; score: number }[];
  } | null>(null);
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="flex items-end gap-2">
                <button
                  onClick={() => run(false)}
                  disabled={running}
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground hover:glow-purple disabled:opacity-50"
                >
                  {running ? (
              ...
              <div className="mt-4 grid gap-2">
                {result.students.length ? (
                  result.students.map((s, i) => (
                    <div
                      key={s.user_id}
                      className="flex items-center justify-between rounded-2xl border border-border bg-secondary/30 px-4 py-3"
                    >
