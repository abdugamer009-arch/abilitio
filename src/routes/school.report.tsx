...
function SchoolReportPage() {
  ...
  useEffect(() => {
    if (loading) return;
    if (!user) { navigate({ to: "/auth", search: { mode: "login", next: "/school/report" } }); return; }
    fetchDashboard().then(setData).catch(() => navigate({ to: "/school/dashboard" }));
    ...
    <div className="min-h-screen bg-white text-slate-900 print:bg-white">
      ...
      <div className="mx-auto max-w-4xl px-10 py-12">
        <div className="no-print mb-6 flex items-center justify-between">
          <button onClick={() => navigate({ to: "/school/dashboard" })} className="text-sm text-slate-500 hover:text-slate-900">← Back to dashboard</button>
          <button onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-full bg-purple-600 px-4 py-2 text-sm text-white hover:bg-purple-700">
            <Printer className="h-4 w-4" /> Print / Save as PDF
          </button>
        </div>

        <header className="border-b border-slate-200 pb-6">
          <div className="text-xs uppercase tracking-widest text-purple-600">Abilitio · School Report</div>
          <h1 className="mt-2 text-3xl font-bold">{data.school.name}</h1>
          <div className="mt-1 text-sm text-slate-500">
            {data.school.city}{data.school.city && data.school.country ? ", " : ""}{data.school.country} · Code <span className="font-mono">{data.school.code}</span>
          </div>
        </header>
        ...
        <section className="mt-8">
          ...
          <table className="mt-3 w-full text-sm">
            <thead><tr className="border-b border-slate-200 text-left text-xs text-slate-500"><th className="py-2">Orientation</th><th>Students</th></tr></thead>
            <tbody>
          ...
          {data.classBreakdown.map((c) => (
            <div key={c.classId} className="mt-4 rounded-xl border border-slate-200 p-4">
              ...
              <div className="mt-2 grid grid-cols-4 gap-2 text-xs">
                {c.buckets.filter((b) => b.count > 0).map((b) => (
        ...
        <section className="mt-8">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {data.insights.map((i, idx) => <li key={idx} className="rounded-lg border border-purple-200 bg-purple-50 px-3 py-2">{i}</li>)}
          </ul>
        </section>
